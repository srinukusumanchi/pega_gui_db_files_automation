@BILLPAYMENTTXN_BATCH_VALIDATION
Feature: Validation of Batch file Smoke scenarios for all channels

  ##########################EMAIL#############################

  @EMAIL  @BILLPAYMENTTXN @5616 @BILLPAYMENTTXN_5616_BATCH_POSITIVE @RS21133
  ...
  @EMAIL  @BILLPAYMENTTXN @5616 @BILLPAYMENTTXN_5616_BATCH_NEGATIVE @RS21133
  Scenario Outline: Bill Payment Txn Email channel Validate the batch file for negative test cases
    Given load data from "BillPaymentTxn" excel sheet
    Given read batch file customer information details from excel sheet "CUST_DIM" for "<Test cases>"
    When read account information details from excel sheet "ACCT_DIM" for "<Test cases>"
    And read Bill Payment Txn "BPTxn" extract file with Preference customer offer details
