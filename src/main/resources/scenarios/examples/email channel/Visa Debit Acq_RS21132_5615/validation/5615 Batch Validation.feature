@VISADEBITACQ_BATCH_VALIDATION
Feature: Validation of Batch file Smoke scenarios for all channels

  ##########################EMAIL#############################

  @EMAIL  @VISADEBITACQ @5616 @VISADEBITACQ_5615_BATCH_POSITIVE @RS21133
  ...
  @EMAIL  @VISADEBITACQ @5616 @VISADEBITACQ_5615_BATCH_NEGATIVE @RS21132
  Scenario Outline: Visa Debit Acquisition Email channel Validate the batch file for negative test cases
    Given load data from "VISADEBITACQ" excel sheet
    Given read batch file customer information details from excel sheet "CUST_DIM" for "<Test cases>"
    When read account information details from excel sheet "ACCT_DIM" for "<Test cases>"
    And read Visa Debit Acq "BPTxn" extract file with Preference customer offer details
